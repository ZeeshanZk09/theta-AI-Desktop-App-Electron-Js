...
import { type DashboardSettings } from "@/types/index";

interface DashboardSettingsModalProps {
  isOpen: boolean;
  onClose: () => void;
  dashboardSettings: Record<string, unknown>;
  handleUpdateDashboardSettings: (settings: DashboardSettings) => void;
}
      ...
      <div className="w-full max-w-lg bg-j-panel border border-j-hologram/30 rounded-[2.5rem] shadow-[0_0_50px_rgba(0,255,163,0.2)] overflow-hidden relative z-10 animate-in zoom-in-95 duration-300">
        <div className="p-8 flex flex-col">
          <div className="flex justify-between items-center mb-8">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-[10px] font-mono text-j-text-muted uppercase tracking-widest">
                  Customize Your Feed
                </p>
              </div>
            </div>
          ...
          <div className="space-y-6">
            <div className="space-y-2">
              ...
              <input
                type="text"
                id="interests"
                value={(dashboardSettings as DashboardSettings).interests.join(", ")}
                onChange={(e) =>
                  handleUpdateDashboardSettings({
                    ...dashboardSettings,
            ...
            <div className="space-y-2">
              ...
              <input
                type="number"
                id="refreshInterval"
                value={(dashboardSettings as DashboardSettings).refreshInterval as string | number}
                onChange={(e) =>
                  handleUpdateDashboardSettings({
                    ...dashboardSettings,
                    refreshInterval: Number.parseInt(e.target.value) || 3600,
          ...
          <div className="mt-8 pt-6 border-t border-white/5 flex items-center justify-between">
            <p className="text-[10px] text-j-text-muted italic">
              Changes will reflect after next refresh.
            </p>
            <button
              onClick={() => {
                playClick();
