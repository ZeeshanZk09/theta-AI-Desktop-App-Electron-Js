...
const UpdateNotification: React.FC = () => {
  ...
  useEffect(() => {
    logger.log("UpdateNotification Component Mounted");
    ...
    electronAPI
      ...
      .then(() => {
        electronAPI
          ?.checkForUpdate()
          .then((res: { success: boolean; version?: string }) => {
            logger.log("Manual check result:", res);
            if (res && res.success && res.version) {
              electronAPI
                ?.getAppVersion()
                .then((currentVer: string) => {
                  // FIX: Only show if Remote Version is GREATER than Current Version
                  if (res.version && isVersionNewer(res.version, currentVer)) {
                    logger.log(`Update confirmed: ${res.version} is newer than ${currentVer}`);
                    setUpdateInfo({ version: res.version });
                    setUpdateState("available");
                    setIsExpanded(true);
                  } else {
                    logger.log(`No valid update: ${res.version} is not newer than ${currentVer}`);
                  }
                })
                .catch(() => {
                  logger.log(`No valid update: ${res.version} is not newer than current version`);
                });
            }
          })
          .catch((err: Record<string, unknown>) => logger.error("Manual check error:", err));
      })
      .catch((err: Record<string, unknown>) =>
        logger.error("Failed to get current app version:", err)
      );

    // Listen for debug logs from Main process
    if (electronAPI?.onUpdateLog) {
      electronAPI.onUpdateLog((message: string) => {
        logger.log("%c[AutoUpdate Main Log]:", "color: cyan", message);
      });
    }
    ...
    const unsubAvailable = electronAPI?.onUpdateAvailable((info: UpdateInfo) => {
      logger.log("EVENT RECEIVED: Update available:", info);
      setUpdateInfo(info);
      setUpdateState("available");
      setIsExpanded(true);
    });

    const unsubProgress = electronAPI?.onUpdateDownloadProgress((progress: DownloadProgress) => {
      logger.log("EVENT RECEIVED: Download progress:", progress);
      setDownloadProgress(progress);
    });

    const unsubDownloaded = electronAPI?.onUpdateDownloaded(() => {
      logger.log("EVENT RECEIVED: Update downloaded");
    ...
    logger.log("Invoking manual check-for-update from UI...");
    electronAPI
      ?.checkForUpdate()
      .then((res: { success: boolean; version?: string }) => {
        logger.log("Manual check result:", res);
          ...
          >
            ...
            <div className="flex items-center justify-between p-4 border-b border-white/5">
              <div className="flex items-center gap-3">
                <div
                  className="w-10 h-10 rounded-xl bg-gradient-to-br from-emerald-500/20 to-cyan-500/20 
              ...
              <p className="text-xs text-j-text-secondary text-center">
                {updateState === "ready"
                  ...
                  : updateState === "downloading"
                    ...
                    : "A new version of Theta AI is available."}
              </p>
            </div>
          </div>,
          document.body
